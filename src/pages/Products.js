...
const Products = () => {
  ...
  // Fetch categories (only show categories that have products)
  useEffect(() => {
    api.getCategories()
      .then(async (data) => {
        ...
        for (const cat of flattened) {
          try {
            const prods = await api.getProducts({ category: cat.slug });
            const items = Array.isArray(prods) ? prods : (prods.data || prods.items || []);
            if (items.length > 0) withProducts.push(cat);
          } catch { /* skip */ }
        }

        setCategories([{ slug: 'all', name: 'Tất cả' }, ...withProducts]);
      })
      .catch(err => {
